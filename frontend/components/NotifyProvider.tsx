...
/* Toast notification provider.  Use:
 *   const notify = useNotify();
 *   notify.success('Account created');
 *   notify.error('Capture rejected', { detail: err.message });
 */
...
export function NotifyProvider({ children }: { children: React.ReactNode }) {
  ...
  /* Sweep expired toasts every 500ms — cheaper than a timer per toast. */
  useEffect(() => {
    const i = setInterval(() => {
      const now = Date.now();
      setToasts((cur) => cur.filter((t) => now - t.ts < t.ttl));
    }, 500);
        ...
        {toasts.map((t) => (
          <div key={t.id} className={`toast toast-${t.kind}`} onClick={() => dismiss(t.id)} role="status">
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              {t.detail ? <div style={{ fontSize: 11.5, color: 'rgb(var(--muted))', marginTop: 2 }}>{t.detail}</div> : null}
            </div>
          </div>
        ))}
      </div>
